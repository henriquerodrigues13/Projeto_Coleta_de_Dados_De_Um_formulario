...
def coleta_de_dados_pessoias_Primeiro_Eletivo(tabela2):
    if tabela2.iloc[5, 1] == 'ENADE' or tabela2.iloc[5, 2] == 'ENADE':
...
def coleta_de_dados_Componetes_Curriculares_Pendentes_NAO_ENADE(tabela2):
    ...
    while i < len(tabela2):
        ...
        informatica = str(tabela2.iloc[i, 2])
        if estatica == 'PROBABILIDADE ESTATÍSTICA':
            ...
            i += 3
        elif informatica == 'CONTABILIDADE APLICADA A INFORMÁTICA':
            componentes_curriculares.append({
                'Componente curricular': 'CONTABILIDADE APLICADA A INFORMATICA',
                'CH': tabela2.iloc[i + 1, 3],
                'Media': tabela2.iloc[i + 1, 6]
            })
            i += 3
        elif tabela2.iloc[i, 2] == '':
            i += 1
        else:
            componentes_curriculares.append({
...
def coleta_de_dados_Componetes_Curriculares_Pendentes_ENADE(tabela2):
    ...
    try:
        while i < len(tabela2):
            estatica = str(tabela2.iloc[i, 3])
            informatica = str(tabela2.iloc[i, 3])
            if estatica == 'PROBABILIDADE ESTATÍSTICA':
                componentes_curriculares.append({
                    'Componente curricular': 'PROBABILIDADE ESTATISTICA',
                    'CH': tabela2.iloc[i + 1, 4],
                    'Media': tabela2.iloc[i + 1, 7]
                })
                i += 3
            elif str(tabela2.iloc[i, 3]) == '':
                i += 1
            elif informatica == 'CONTABILIDADE APLICADA A INFORMÁTICA':
                componentes_curriculares.append({
                    'Componente curricular': 'CONTABILIDADE APLICADA A INFORMATICA',
                ...
                componentes_curriculares.append({
                    'Componente curricular': tabela2.iloc[i, 3],
                    'CH': tabela2.iloc[i + 1, 4],
                    'Media': tabela2.iloc[i + 1, 7]
                })
                i += 3
    except IndexError:
        coluna_vazia = str(tabela2.iloc[i, 7])
        if coluna_vazia != '':
            componentes_curriculares.append({
                'Componente curricular': tabela2.iloc[i, 3],
                'CH': tabela2.iloc[i, 4],
                'Media': tabela2.iloc[i, 7]
            })
    return componentes_curriculares
...
if tables is not None:
    for i in range(0, len(tables)):
        ...
        pedente1 = Terceira_tabela.iloc[0, 2]
        pedente2 = Terceira_tabela.iloc[0, 1]
